...
Jika kamu mengubah kode lokal dan setelah `makepkg -si` perubahan tidak muncul di sistem,

catatan : ini bukan cara yang benar, tapi ini berhasil :)

1) hapus package `hypryou-2.1.5....pkg.tar` dan folder `hyprland-material-you` hasil build sebelumnya
2) (optional) naikan `pkgrel` di `PKGBUILD` (misalnya dari `pkgrel=1` ke `pkgrel=2`)
3) hapus instalasi lama <br>
`sudo rm -rf /usr/lib/hypryou /usr/share/hypryou /usr/bin/hypryou* /usr/share/wayland-sessions/hypryou.desktop`
3) rebuild package dengan `makepkg -si` di root project
4) periksa apakah file yang kamu ubah sudah ter-copy dengan benar, <br>
   kamu bisa cek di folder `ls /usr/lib/hypryou` atau `ls /usr/share/hypryou`
